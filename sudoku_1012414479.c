   ...
   * *****/
   ...
   for(int i = 0; i < 9; i +=3){
    for (int j = 0; j <9 ; j +=3){
      ...
      if(sudoku[n][m] < 1 || sudoku[n][m] > 9){
        return 0;
      }
      
        
      
      if(check[sudoku[n][m]] == 1){
        return 0;
        }
        check[sudoku[n][m]] = 1;
       }
...
int solve_helper(int sudoku[9][9]){
  int row = -1;
  int col = -1;
  int find_zero = 0;
  for(int i = 0;  i < 9 && find_zero == 0; i++){
    for(int j = 0; j<9;j++){
      if (sudoku[i][j] == 0){
        row = i;
        col = j;
  ...
  sudoku[row][col] = temp;
  if (check_sudoku(sudoku) == 1 && solve_helper(sudoku) == 1){
    return 1;
  }
  sudoku[row][col] = 0;
  }
